 ...
 * Game page
 */
public class QuickStartRuntimeActivity extends BaseActivity {
    ...
    public static void start(Context context, GameModel model) {
        Intent intent = new Intent(context, QuickStartRuntimeActivity.class);
    ...
    protected void setListeners() {
        ...
        gameViewModel.progressLiveData.observe(this, new Observer<BaseRuntimeGameViewModel.ProgressModel>() {
            @Override
            public void onChanged(BaseRuntimeGameViewModel.ProgressModel model) {
    ...
    private void updateProgress(BaseRuntimeGameViewModel.ProgressModel model) {
        if (model == null) {
            return;
        }
        if (model.stage == 3) {
